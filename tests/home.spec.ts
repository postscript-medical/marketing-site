...
test.describe("Homapage", () => {
  ...
  test("Homepage - Visual check", async ({ page, browserName }) => {
  ...
  test.describe("Footer navigation", () => {
    ...
    test("Test Twitter link", async ({ page }) => {
      await expect(page.getByRole('link', { name: 'Twitter' })).toHaveAttribute('href', 'https://twitter.com/postscriptmed');
    });

    test("Test LinkedIn link", async ({ page }) => {
      await expect(page.getByRole('link', { name: 'LinkedIn' })).toHaveAttribute('href', 'https://www.linkedin.com/company/postscriptmedical/');
    });
  });
